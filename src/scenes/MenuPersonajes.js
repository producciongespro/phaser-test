...
export default class MenuPersonajes extends Scene {
  ...
  init() {
    ...
    this.estilosTexto = this.registry.get("estilosTexto"); //Estilos de textos
  ...
  create() {
    ...
    const btnFlechaIzquierdaVerde = this.add
      .image(this.center.x - 290, this.center.y, "btnFlechaIzquierdaVerde")
      .setOrigin(0.5)
      .setInteractive();
    const btnFlechaDerechaVerde = this.add
      .image(this.center.x + 290, this.center.y, "btnFlechaDerechaVerde")
      .setOrigin(0.5)
      .setInteractive();
    ...
    this.add
      .image(this.center.x, this.center.y, "bgPersonajes2")
      .setOrigin(0.5);
  }
  ...
  update() {
    ...
    this.add
      .text(
        this.center.x,
        this.center.y + 200,
        this.personajes[this.indice].nombre,
        this.estilosTexto.negro
      )
      .setOrigin(0.5);
    this.add
      .image(this.center.x, this.center.y + 100, "btnLapiz")
      .setOrigin(0.5)
      .setInteractive();
  }
  ...
  renderPersonajes(i) {
    ...
    this.personajeIzquierda = this.add
      .image(
        this.center.x - 120,
        this.center.y,
        this.personajes[i - 1 < 0 ? this.personajes.length - 1 : i - 1].id
      )
      .setScale(0.4);
    this.personajeCentral = this.add
      .image(this.center.x, this.center.y, this.personajes[i].id)
      .setScale(0.9);
    this.personajeDerecha = this.add
      .image(
        this.center.x + 120,
        this.center.y,
        this.personajes[(i + 1) % this.personajes.length].id
      )
      .setScale(0.4);

    // Hacer que el personaje central sea interactivo
    this.personajeCentral.setInteractive();

    // Agregar el evento pointerdown para obtener el ID del personaje central
    this.personajeCentral.on("pointerdown", () => handleSeleccionarPersonaje());
  }

  handleSeleccionarPersonaje() {
    const personajeID = this.personajes[i].id; // Obtener el ID del personaje actual
  ...
  handleAvanzar(direccion) {
    if (direccion === "d") {
      ...
      this.indice =
        (this.indice - 1 + this.personajes.length) % this.personajes.length;
    }
    ...
    this.tweens.add({
      ...
      duration: 500,
      onComplete: (tween) => {
        tween.remove(); // Remueve el tween una vez que ha terminado
